...
;;;###autoload
(defcustom ein:jupyter-server-use-subcommand "server"
  "Subcommand for jupyter server.  \"notebook\" for classic notebook server,
\"server\" for Jupyter Server >= 2.0.  Nil for standalone jupyter-notebook."
  :group 'ein
  :type '(choice (string :tag "Subcommand" "server")
                 (const :tag "Omit" nil)))
...
of (PASSWORD TOKEN)."
  (aif (cl-loop for line in
                ...
                with token0
                with password0
                for json = (ein:json-read-from-string line)
                for json-url = (lax-plist-get json :url)
                for json-port = (lax-plist-get json :port)
                for target-port = (ignore-errors
                                    (string-to-number
                                     (or (url-port
                                         ...
                                         "")))
                when (and json-url
                          (or (equal (ein:url json-url)
                                     (ein:url url-or-port))
                              (and (integerp json-port)
                                   (integerp target-port)
                                   (= json-port target-port))))
                do (setq password0 (lax-plist-get json :password))
                (setq token0 (lax-plist-get json :token))
                and return (list password0 token0))
      it (list nil nil)))
