...
ein:notebooklist-open*."
  ...
  (let ((token (or token (ein:notebooklist-token-or-password url-or-port))))
    (cond ((string= token "") ;; all authentication disabled
           (ein:log 'verbose "Skipping login %s" url-or-port)
           (ein:notebooklist-open* url-or-port nil nil callback nil))
          ((or (null token) ;; don't know - crib unavailable
               (ein:query-api-version--jupyter-server-p url-or-port))
           ;; Jupyter Server uses token auth via URL, no /login endpoint.
           ;; Open directly; ein:notebooklist-open* will detect version via /api/status.
           (ein:log 'verbose "Skipping /login for %s" url-or-port)
           (ein:notebooklist-open* url-or-port nil nil callback nil))
          (t
