...
Tries /api/status first (Jupyter Server >= 2.x), falls back to api/spec.yaml
(classic notebook server)."
  (ein:query-singleton-ajax
   (ein:url url-or-port "api/status")
   :type "GET"
   :parser (lambda ()
             (let* ((json (ein:json-read-from-string
                           (buffer-substring-no-properties (point-min) (point-max))))
                    (version (plist-get json :version)))
               (if version
                   (number-to-string (truncate (string-to-number version)))
                 "")))
   :complete (apply-partially #'ein:query-notebook-api-version--status-complete
                              url-or-port callback)))

(defun ein:query-notebook-api-version--fallback (url-or-port callback)
  "Fallback to old api/spec.yaml for classic notebook server."
  (ein:query-singleton-ajax
...
(defun ein:query-api-version--jupyter-server-p (url-or-port)
  "Return non-nil if URL-OR-PORT is a Jupyter Server >= 2.x."
  (let ((version (gethash url-or-port *ein:notebook-api-version*)))
    (and version (string= version "6"))))

(cl-defun ein:query-notebook-api-version--status-complete
    (url-or-port callback
     &key data response
     &allow-other-keys &aux
     (status (request-response-status-code response))
     (resp-string (format "STATUS: %s DATA: %s" status data)))
  (ein:log 'debug "ein:query-notebook-api-version--status-complete %s" resp-string)
  (if (and (not (zerop (string-to-number data))) (not (eql status 404)))
      ;; Jupyter Server returns version "2.x" from /api/status.
      ;; Normalize to api-version "6" for EIN's internal version checks.
      (setf (gethash url-or-port *ein:notebook-api-version*)
            (if (>= (string-to-number data) 2) "6" data))
    (ein:log 'warn "ein:query-notebook-api-version--status-complete: /api/status failed, trying api/spec.yaml")
    (ein:query-notebook-api-version--fallback url-or-port callback)))
